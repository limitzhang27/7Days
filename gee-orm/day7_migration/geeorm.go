...
func (engine *Engine) Migrate(value interface{}) error {
	_, err := engine.Transaction(func(s *session.Session) (result interface{}, err error) {
		if !s.Model(value).HasTable() {
			log.Infof("table %s dozen`t exist", s.RefTable().Name)
		...
		log.Info("added cols %v, deleted cols %s", addCols, delCols)
		...
		tmp := "tmp_" + table.Name
		fieldString := strings.Join(table.FieldNames, ", ")
		s.Raw(fmt.Sprintf("CREATE TABLE %s AS SELECT %s FROM %s;", tmp, fieldString, table.Name))
